...
def sample_folder(tmp_path):
    ...
    (tmp_path / "d.txt").write_text("x")  # unsupported, should be ignored
...
def test_find_batch_files_only_supported_extensions(sample_folder):
    ...
    assert names == ["a.csv", "b.xlsx", "c.kmz", "e.kml"]
...
def test_run_batch_continues_after_a_failure(sample_folder):
    ...
    assert len(report.results) == 4  # all 4 supported files were attempted
