...
def _cluster_axis(values: List[float], tolerance: float | None = None) -> List[List[int]]:
    ...
    for idx in order[1:]:
        ...
        else: groups.append([idx]); centers.append(values[idx])
    return groups
...
def order_points(points: Iterable[PointResult], mode: str="SOURCE", tolerance: float|None=None, reverse: bool=False) -> List[OrderedPoint]:
    """SOURCE preserves file order; GRID_ZIGZAG detects rows from Y and alternates X."""
    pts=list(points); mode=mode.upper()
    if mode=="SOURCE": ordered=[(p,0,0) for p in pts]
    else:
        ...
        if mode in {"X_ASC","EAST_WEST"}: ordered=[(p,0,0) for p in sorted(valid,key=lambda p:(_xy(p)[0],_xy(p)[1]))]
        elif mode in {"X_DESC","WEST_EAST"}: ordered=[(p,0,0) for p in sorted(valid,key=lambda p:(-_xy(p)[0],_xy(p)[1]))]
        elif mode=="Y_ASC": ordered=[(p,0,0) for p in sorted(valid,key=lambda p:(_xy(p)[1],_xy(p)[0]))]
        elif mode=="Y_DESC": ordered=[(p,0,0) for p in sorted(valid,key=lambda p:(-_xy(p)[1],_xy(p)[0]))]
        else:
            groups=_cluster_axis([_xy(p)[1] for p in valid],tolerance); rows=[]
            groups=sorted(groups,key=lambda g:sum(_xy(valid[i])[1] for i in g)/len(g),reverse=True)
            for r,g in enumerate(groups,1):
                row=[valid[i] for i in g]; row.sort(key=lambda p:_xy(p)[0],reverse=(r%2==0)); rows.extend((p,r,c) for c,p in enumerate(row,1))
