...
def export_xlsx(
    ...
    for i, p in enumerate(points, start=1):
        row = [i, p.name, p.src_x, p.src_y, p.src_z, p.tgt_x, p.tgt_y, p.tgt_z, f"{p.tgt_x:.{precision}f}, {p.tgt_y:.{precision}f}" if p.tgt_x is not None and p.tgt_y is not None else "", p.status]
        for j, v in enumerate(row, start=1):
            cell = ws.cell(row=i + 1, column=j, value=v); cell.border = BORDER
    ...
    rows = [
        ("Source CRS", source_crs), ("Target CRS", target_crs), ("Transformation", "PROJ (pyproj.Transformer)"),
        ...
        ("Number of Points", len(points)), ("Date", date.today().isoformat()), ("Decimal Precision", precision),
