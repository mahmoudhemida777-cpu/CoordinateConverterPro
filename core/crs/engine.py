...
class _AmanahLocalTransformer:
    ...
    def forward(cls, x: float, y: float) -> tuple[float, float]:
        return cls._TX + cls._Q00 * x + cls._Q10 * y, cls._TY + cls._Q01 * x + cls._Q11 * y
    ...
    def inverse_transform(cls, x: float, y: float) -> tuple[float, float]:
        ...
        return (cls._Q11 * dx - cls._Q10 * dy) / det, (-cls._Q01 * dx + cls._Q00 * dy) / det

    def __init__(self, use_inverse: bool = False) -> None:
        self.use_inverse = use_inverse

    def transform(self, x: float, y: float, z: Optional[float] = None):
        tx, ty = self.inverse_transform(x, y) if self.use_inverse else self.forward(x, y)
...
class CRSEngine:
    ...
    @property
    def authorities(self) -> List[str]: return list(get_authorities())

    def catalog(self, include_deprecated: bool = False) -> List[CRSInfo]:
        if self._catalog_cache is not None and not include_deprecated: return list(self._catalog_cache)
        ...
        if not include_deprecated: self._catalog_cache = results
        return list(results)

    def search(self, query: str, limit: int = 50) -> List[CRSInfo]:
        query_norm = query.strip()
        if not query_norm: return self.catalog()[:limit]
        key = " ".join(query_norm.lower().split()); results: List[CRSInfo] = []
        ...
        if alias == self.AMANAH_RIYADH:
            ...
            try:
                crs = CRS.from_user_input(alias); auth, code = self._split_authority_code(alias)
        ...
        if ":" in authority_code and authority_code != self.AMANAH_RIYADH:
            ...
            try:
                crs = CRS.from_user_input(f"{auth}:{code}"); results.append(CRSInfo(auth, code, crs.name, crs.type_name))
            except Exception: pass
        elif query_norm.isdigit():
            for item in self.catalog():
                if item.code == query_norm: results.append(item)
                if len(results) >= limit: return self._dedupe(results)[:limit]
        if "amanah" in key or "الأمانة" in key or "امانة" in key:
            results.append(CRSInfo("MHLOCAL", "AMANAH-RYD-LOCAL-38N", self.AMANAH_NAME, "PROJECTED_CRS"))
        for item in self.catalog():
            if key in item.name.lower() or key in item.epsg.lower(): results.append(item)
            if len(results) >= limit: return self._dedupe(results)[:limit]
        return self._dedupe(results)[:limit]

    @staticmethod
    def _split_authority_code(identifier: str) -> tuple[str, str]: return identifier.split(":", 1)[0].upper(), identifier.split(":", 1)[1]
    ...
    def _resolve_crs(self, identifier: str) -> CRS:
        ident = identifier.strip(); alias = self._CRS_ALIASES.get(" ".join(ident.lower().split()))
        if alias: ident = alias
        if ident.upper() == self.AMANAH_RIYADH: raise CRSError(f"'{self.AMANAH_RIYADH}' is a project-local CRS; use the engine transformation API.")
        try: return CRS.from_user_input(ident)
        except Exception as exc: raise CRSError(f"Could not resolve CRS '{identifier}': {exc}") from exc
    ...
    def get_crs_details(self, epsg_or_code: str) -> dict:
        if self._is_amanah(epsg_or_code):
            return {"name": self.AMANAH_NAME, "epsg": self.AMANAH_RIYADH, "datum": "Ain el Abd 1970 + project local adjustment", "ellipsoid": "International 1924 (Hayford 1909)", "projection": "UTM Zone 38N + fitted 2D similarity adjustment", "units": "metre", "is_projected": True, "is_geographic": False}
        crs = self._resolve_crs(epsg_or_code); datum = crs.datum; ellipsoid = datum.ellipsoid if datum else None; axis_units = crs.axis_info[0].unit_name if crs.axis_info else "unknown"
        return {"name": crs.name, "epsg": f"EPSG:{crs.to_epsg()}" if crs.to_epsg() else str(crs.srs), "datum": datum.name if datum else "N/A", "ellipsoid": ellipsoid.name if ellipsoid else "N/A", "projection": crs.coordinate_operation.method_name if crs.coordinate_operation else "Geographic (no projection)", "units": axis_units, "is_projected": crs.is_projected, "is_geographic": crs.is_geographic}

    def validate_point_domain(self, crs_identifier: str, x: float, y: float) -> tuple[bool, str]:
        if self._is_amanah(crs_identifier): return self.validate_point_domain(self.AMANAH_BASE, x, y)
        crs = self._resolve_crs(crs_identifier)
        if not (float(x) == float(x) and float(y) == float(y)): return False, "Coordinate contains NaN."
        try:
            if crs.is_geographic: lon, lat = float(x), float(y)
            else:
                inv = Transformer.from_crs(crs, CRS.from_epsg(4326), always_xy=True); lon, lat = inv.transform(float(x), float(y))
            if not (-180 <= lon <= 180 and -90 <= lat <= 90): return False, f"Coordinate resolves outside Earth bounds ({lon:.6f}, {lat:.6f})."
            area = crs.area_of_use
            if area and not (area.west <= lon <= area.east and area.south <= lat <= area.north): return False, f"Coordinate is outside the selected CRS area of use ({lon:.6f}, {lat:.6f})."
        except Exception as exc: return False, f"Could not validate coordinate for CRS: {exc}"
        return True, ""

    def _custom_transformer(self, source: str, target: str):
        src_a, tgt_a = self._is_amanah(source), self._is_amanah(target)
        if src_a and tgt_a: return _AmanahLocalTransformer(False)
        if src_a:
        ...
        source_to_base = Transformer.from_crs(self._resolve_crs(source), self._resolve_crs(self.AMANAH_BASE), always_xy=True)
        if tgt_a: return _ComposedTransformer(source_to_base, _AmanahLocalTransformer(False))
        raise CRSError("Internal custom CRS routing error")

    def get_operations(self, source: str, target: str) -> List[dict]:
        key = (source, target)
        if key in self._operation_cache: return list(self._operation_cache[key])
        if self._is_amanah(source) or self._is_amanah(target):
            direction = "Amanah Riyadh Local Grid 38N → Ain el Abd 1970 / UTM 38N" if self._is_amanah(source) else "Ain el Abd 1970 / UTM 38N → Amanah Riyadh Local Grid 38N"
            if not self._is_amanah(source) and not self._is_amanah(target): direction = "PROJ"
            operations = [{"id": 0, "name": "Amanah Riyadh Local Grid 38N", "description": f"{direction} (5-control-point 2D similarity fit)", "accuracy": 0.003}]
            self._operation_cache[key] = operations; return list(operations)
        group = TransformerGroup(self._resolve_crs(source), self._resolve_crs(target), always_xy=True, allow_ballpark=False)
        operations = [{"id": idx, "name": transformer.name, "description": transformer.description, "accuracy": transformer.accuracy} for idx, transformer in enumerate(group.transformers)]
        self._operation_cache[key] = operations; return list(operations)
    ...
    def get_transformer(self, source: str, target: str, operation: str = "auto"):
        ...
        if key not in self._transformer_cache:
            if self._is_amanah(source) or self._is_amanah(target):
                if operation not in ("auto", "0"): raise CRSError(f"Unknown Amanah local transformation operation '{operation}'.")
                self._transformer_cache[key] = self._custom_transformer(source, target)
            else:
                group = TransformerGroup(self._resolve_crs(source), self._resolve_crs(target), always_xy=True, allow_ballpark=False); transformers = list(group.transformers)
                if not transformers: raise CRSError(f"No non-ballpark transformation available from {source} to {target}.")
                if operation == "auto": selected = transformers[0]
                else:
                    try: selected = transformers[int(operation)]
                    except (ValueError, IndexError): raise CRSError(f"Unknown transformation operation '{operation}'.")
    ...
    def get_selected_operation(self, source: str, target: str, operation: str = "auto") -> dict:
        ...
        if not operations: return {"id": None, "name": "N/A", "description": "No operation", "accuracy": None}
        if operation == "auto": return operations[0]
        try: return operations[int(operation)]
        except (ValueError, IndexError): raise CRSError(f"Unknown transformation operation '{operation}'.")
    ...
    def transform_points(self, source: str, target: str, points: Sequence[PointResult], operation: str = "auto") -> List[PointResult]:
        ...
        for p in points:
            ...
            try:
                ...
                if any(v != v for v in (tx, ty)): raise ValueError("Transformation returned NaN (point likely outside CRS domain)")
                target_valid, target_reason = self.validate_point_domain(target, tx, ty)
                if not target_valid: raise ValueError(f"Target CRS validation failed: {target_reason}")
                out.append(PointResult(p.name, p.src_x, p.src_y, p.src_z, tx, ty, tz, "SUCCESS", ""))
            except Exception as exc: out.append(PointResult(p.name, p.src_x, p.src_y, p.src_z, status="FAILED", message=str(exc)))
        return out

    def proj_version(self) -> str: return pyproj.proj_version_str
