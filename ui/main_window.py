...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        if app is not None:
            set_language(str(__import__('PySide6.QtCore', fromlist=['QSettings']).QSettings("Mahmoud Hemida", "MH GeoSuite Pro").value("language", "en")))
            apply_theme(app)
        ...
        self.resize(1440,900); self.setMinimumSize(1100,700); self.workspace_folder=None
        central=QWidget(); self.setCentralWidget(central)
        layout=QHBoxLayout(central); layout.setContentsMargins(0,0,0,0); layout.setSpacing(0)
        self.sidebar=QListWidget(); self.sidebar.setFixedWidth(238); self.sidebar.setObjectName("sidebar"); self.sidebar.setIconSize(QSize(20,20)); self.sidebar.setSpacing(2)
        self.sidebar.setUniformItemSizes(True)
        for key,label,pix in SIDEBAR_ITEMS:
            item=QListWidgetItem(self.style().standardIcon(pix),tr(label)); item.setData(Qt.ItemDataRole.UserRole,key); item.setSizeHint(QSize(214,45)); item.setData(Qt.ItemDataRole.UserRole+1,label); self.sidebar.addItem(item)
    ...
    def _on_language_changed(self, language: str) -> None:
        app=QApplication.instance()
        if app is not None:
            app.setLayoutDirection(Qt.LayoutDirection.RightToLeft if language=="ar" else Qt.LayoutDirection.LeftToRight)
        ...
        self.workspace_banner.setLayoutDirection(Qt.LayoutDirection.RightToLeft if language=="ar" else Qt.LayoutDirection.LeftToRight)
