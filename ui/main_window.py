...
from PySide6.QtWidgets import (
    ...
    QListWidgetItem, QStackedWidget, QStyle, QApplication, QAbstractButton,
    QGroupBox, QComboBox, QLineEdit, QPlainTextEdit, QTableWidget,
)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        if app is not None:
            settings = QSettings("Mahmoud Hemida", "MH GeoSuite Pro")
            set_language(str(settings.value("language", "ar") or "ar"))
            apply_theme(app)
        ...
        for key, label, pix in SIDEBAR_ITEMS:
            item = QListWidgetItem(self.style().standardIcon(pix), tr(label))
            item.setData(Qt.ItemDataRole.UserRole, key)
            item.setData(Qt.ItemDataRole.UserRole + 1, label)
            item.setSizeHint(QSize(214, 45))
        ...
        self.pages["import"].file_loaded.connect(self._set_active_file)
        self.pages["batch"].batch_completed.connect(self._on_batch_completed)
        self.pages["history"].file_reloaded.connect(self._reload_history_file)
    ...
    def _translate_text(self, text: str) -> str:
        if current_language() == "ar":
            return tr(text)
        reverse = {value: key for key, value in _TRANSLATIONS.get("ar", {}).items()}
        ...
        for table in self.findChildren(QTableWidget):
            for index in range(table.columnCount()):
                ...
                if item is not None and item.text():
                    item.setData(Qt.ItemDataRole.UserRole + 2, item.text())
    ...
    def _set_workspace_folder(self, folder: str):
        ...
        for page in self.pages.values():
            ...
            if callable(setter):
                setter(folder)
        self.statusBar().showMessage(f"{tr('Project Folder')}: {folder} — {tr('shared workspace')}")
    ...
    def _set_active_file(self, path: str):
        for key in ("import", "converter", "cad", "batch", "map"):
            ...
            if callable(loader):
                loader(path)
        self.statusBar().showMessage(f"{tr('Active File')}: {path}")
    ...
    def _on_sidebar_changed(self, row: int):
        if 0 <= row < len(SIDEBAR_ITEMS):
            self.stack.setCurrentWidget(self.pages[SIDEBAR_ITEMS[row][0]])
