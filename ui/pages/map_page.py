...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QFileDialog, QGraphicsView, QGraphicsScene, QGraphicsEllipseItem, QMessageBox
...
class MapPage(QWidget):
    ...
    def __init__(self) -> None:
        ...
        row = QHBoxLayout()
        btn = QPushButton("Open Coordinate File")
    ...
    def _open(self) -> None:
        path, _ = QFileDialog.getOpenFileName(self, "Open Coordinate File", "", "Supported files (*.csv *.xlsx *.kml *.kmz)")
        if not path:
            return
        try:
            ...
            self._draw(valid)
            self.info.setText(f"{Path(path).name} — {len(valid)} points")
