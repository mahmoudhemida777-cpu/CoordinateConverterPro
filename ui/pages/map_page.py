...
class MapPage(QWidget):
    ...
    def __init__(self)->None:
        ...
        self.scene=QGraphicsScene(self); self.view=QGraphicsView(self.scene); self.view.setSizePolicy(QSizePolicy.Policy.Expanding,QSizePolicy.Policy.Expanding); self.view.setMinimumHeight(560); self.view.setRenderHint(self.view.renderHints()); self.view.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded); self.view.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded); root.addWidget(self.view,1)
    ...
    def _draw(self,points)->None:
        ...
        # Use the actual viewport size so the map occupies the available page instead of a fixed 900x600 box.
        viewport=self.view.viewport().size(); width=max(float(viewport.width()),1000.0); height=max(float(viewport.height()),620.0); margin=60.0
