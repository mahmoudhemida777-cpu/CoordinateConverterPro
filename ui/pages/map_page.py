...
class MapPage(QWidget):
    ...
    def __init__(self) -> None:
        super().__init__(); root = QVBoxLayout(self); root.setContentsMargins(30, 20, 30, 20)
        ...
        row = QHBoxLayout(); btn = QPushButton("Open Coordinate File"); btn.clicked.connect(self._open); row.addWidget(btn)
    ...
    def load_active_file(self, path: str) -> None:
        """Load Dashboard-selected coordinate file without opening another picker."""
    ...
    def _draw(self, points) -> None:
        self.scene.clear(); min_x = min(p[1] for p in points); max_x = max(p[1] for p in points); min_y = min(p[2] for p in points); max_y = max(p[2] for p in points)
        dx = max(max_x-min_x, 1e-9); dy = max(max_y-min_y, 1e-9); width, height, margin = 900.0, 600.0, 40.0
