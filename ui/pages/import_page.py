...
class ImportPage(QWidget):
    ...
    def __init__(self) -> None:
        ...
        title = QLabel(tr("Import")); title.setStyleSheet("font-size: 20px; font-weight: bold; color: #1F3864;")
        ...
        btn_row.addWidget(self.excel_btn); btn_row.addWidget(self.csv_btn); btn_row.addWidget(self.choose_btn); btn_row.addStretch()
        layout.addLayout(btn_row)

        self.file_label = QLabel("No file selected")
        self.file_label.setStyleSheet("color: #777; margin-top: 6px;")
        layout.addWidget(self.file_label)
        self.table = QTableWidget(0, 4); self.table.setHorizontalHeaderLabels(["Name", "X", "Y", "Z"])
        layout.addWidget(self.table)

    def load_active_file(self, path: str) -> None:
        """Load a file selected from Dashboard/Workspace without opening a second picker."""
        if path and Path(path).is_file():
            self._import_path(path)

    def _choose_excel(self) -> None:
        path, _ = QFileDialog.getOpenFileName(self, "Load Excel", "", "Excel files (*.xlsx *.xls)")
    ...
    def _choose_file(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            self, tr("Choose File"), "",
            "Supported files (*.kmz *.kml *.csv *.xlsx *.xls);;KMZ (*.kmz);;KML (*.kml);;CSV (*.csv);;Excel (*.xlsx *.xls)"
        )
        if path: self._import_path(path)
    ...
    def _import_path(self, path: str) -> None:
        ...
        self.active_path = path; self.points = points; self.file_label.setText(path)
        self._populate_table(points); self.points_imported.emit(points)

    def _ask_mapping(self, columns: list[str]) -> dict | None:
        dlg = ColumnMappingDialog(columns, self)
        return dlg.result_mapping() if dlg.exec() == QDialog.Accepted else None

    def _populate_table(self, points: list[PointResult]) -> None:
        self.table.setRowCount(len(points))
        for i, p in enumerate(points):
            self.table.setItem(i, 0, QTableWidgetItem(p.name))
            self.table.setItem(i, 1, QTableWidgetItem("" if p.src_x is None else str(p.src_x)))
            self.table.setItem(i, 2, QTableWidgetItem("" if p.src_y is None else str(p.src_y)))
            self.table.setItem(i, 3, QTableWidgetItem("" if p.src_z is None else str(p.src_z)))
