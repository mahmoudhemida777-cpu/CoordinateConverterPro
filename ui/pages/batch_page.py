...
class BatchPage(QWidget):
    ...
    def __init__(self):
        ...
        self.progress_label=QLabel("Choose a folder, files, or use the Dashboard Active File."); root.addWidget(self.progress_label); self.progress=QProgressBar(); root.addWidget(self.progress)
        self.results_list=QListWidget(); root.addWidget(self.results_list); self.summary_label=QLabel(""); root.addWidget(self.summary_label)
    ...
    def _display_files(self,files,source):
        self.selected_files=list(files); self.results_list.clear(); self.progress.setValue(0); self.progress.setMaximum(max(len(files),1)); self.folder_label.setText(source)
    ...
    def _run_batch(self):
        ...
        try:
            for i,path in enumerate(files,1):
                ...
                try:
                    points=self._parse_file(path); transformed=self.engine.transform_points(src,tgt,points); out=path.with_name(path.stem+"_converted.xlsx"); export_xlsx(transformed,str(out),src,tgt)
                    ok=sum(p.status=="SUCCESS" for p in transformed); bad=sum(p.status=="FAILED" for p in transformed); status="SUCCESS" if bad==0 else ("WARNING" if ok else "FAILED"); r=FileResult(str(path),status,len(transformed),ok,bad,message=str(out)); results.append(r)
            ...
            good=sum(r.status=="SUCCESS" for r in results); bad=sum(r.status=="FAILED" for r in results); warn=sum(r.status=="WARNING" for r in results); self.summary_label.setText(f"Files: {len(results)}   Success: {good}   Warnings: {warn}   Failed: {bad}")
