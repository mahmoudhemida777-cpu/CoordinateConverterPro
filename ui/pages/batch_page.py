...
class BatchPage(QWidget):
    ...
    def __init__(self):
        ...
        crs.addWidget(self.source_picker)
        crs.addWidget(self.target_picker)
        root.addLayout(crs)
        ...
        self.progress_label = QLabel("Choose a folder, files, or use the Dashboard Active File.")
    ...
    def load_active_file(self, path: str):
        ...
        if p.is_file():
            self.active_file = p
            self.folder = None
            self._display_files([p], f"Active File: {p.name}")
            self.workspace_label.setText(f"Workspace Active File: {p.name}")
        else:
            self.active_file = None
    ...
    def _display_files(self, files, source):
        ...
        self.folder_label.setText(source)
        self.progress_label.setText(f"{len(files)} file(s) ready for CRS conversion")
        for p in files:
            self.results_list.addItem(QListWidgetItem(f"READY — {p.name} | {p}"))
    ...
    def _scan_folder(self):
        try:
            self._display_files(find_batch_files(self.folder), f"Folder: {self.folder}")
    ...
    def _choose_files(self):
        ...
        if paths:
            self.folder = None
            self.active_file = None
            self._display_files([Path(p) for p in paths if Path(p).is_file()], f"Selected files: {len(paths)}")
    ...
    def _parse_file(self, path):
        ...
        if s == ".csv":
            c = csv_parser.sniff_columns(str(path))
            return csv_parser.parse_csv(str(path), csv_parser.ColumnMapping(c[0], c[1], c[2], c[3] if len(c) > 3 else None))
        if s == ".xlsx":
            c = xlsx_parser.sniff_columns(str(path))
            return xlsx_parser.parse_xlsx(str(path), xlsx_parser.ColumnMapping(c[0], c[1], c[2], c[3] if len(c) > 3 else None))
    ...
    def _run_batch(self):
        ...
        if not files:
            QMessageBox.warning(self, "No files", "No supported coordinate files selected.")
            return
        ...
        try:
            for i, path in enumerate(files, 1):
                ...
                self.progress_label.setText(f"File {i} / {len(files)}: {path.name}")
                try:
                    ...
                    export_xlsx(transformed, str(out), src, tgt)
                    ok = sum(p.status == "SUCCESS" for p in transformed)
                    bad = sum(p.status == "FAILED" for p in transformed)
                    status = "SUCCESS" if bad == 0 else ("WARNING" if ok else "FAILED")
                    r = FileResult(str(path), status, len(transformed), ok, bad, message=str(out))
                    results.append(r)
                    if status == "SUCCESS":
                    ...
                    if status in ("SUCCESS", "WARNING"):
                        append_history({
                            ...
                            "source_crs": src,
                            "target_crs": tgt,
                            "points": len(transformed),
                            "operation": "Batch Conversion",
                            "status": status,
                            "output": str(out),
                        })
                except Exception as e:
                    results.append(FileResult(str(path), "FAILED", message=str(e)))
            for r in results:
                self.results_list.addItem(QListWidgetItem(f"[{r.status}] {Path(r.path).name} — {r.points_success}/{r.points_total} points {r.message}"))
            good = sum(r.status == "SUCCESS" for r in results)
            bad = sum(r.status == "FAILED" for r in results)
            warn = sum(r.status == "WARNING" for r in results)
            self.summary_label.setText(f"Files: {len(results)}   Success: {good}   Warnings: {warn}   Failed: {bad}")
            self.last_output_paths = outputs
            if outputs:
                self.batch_completed.emit(outputs, tgt, src)
                self.progress_label.setText(f"Batch complete — {len(outputs)} converted file(s) propagated to CAD/Civil 3D.")
        finally:
