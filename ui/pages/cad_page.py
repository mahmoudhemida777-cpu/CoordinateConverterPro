...
class CadPage(QWidget):
    ...
    def __init__(self) -> None:
        ...
        root=QVBoxLayout(self); root.setContentsMargins(30,20,30,20); title=QLabel("AutoCAD / Civil 3D Export"); title.setStyleSheet("font-size:20px;font-weight:bold;color:#1F3864;"); root.addWidget(title); root.addWidget(QLabel("Convert coordinates, then export a ready-to-open DXF or Civil 3D CSV point file."))
    ...
    def load_active_file(self,path:str)->None:
        """Load Dashboard-selected coordinate file without another picker."""
    ...
    def _load_path(self,path:str)->None:
        ...
        self.source_points=points; self.result_points=[]; self.current_file=path; self.file_label.setText(f"{Path(path).name} — {len(points)} points loaded"); self.table.setRowCount(0); self.total.setText(f"Points: {len(points)}"); self.success.setText("Success: 0"); self.failed.setText("Failed: 0")
    ...
    def _export_dxf(self)->None:
        if not self.result_points: QMessageBox.warning(self,"Nothing to export","Run the conversion first."); return
