...
class CadPage(QWidget):
    ...
    def __init__(self) -> None:
        ...
        self.progress=QProgressBar(); root.addWidget(self.progress); summary=QGroupBox(); sl=QHBoxLayout(summary); self.total=QLabel("Points: 0"); self.success=QLabel("Success: 0"); self.failed=QLabel("Failed: 0"); [sl.addWidget(w) for w in (self.total,self.success,self.failed)]; root.addWidget(summary)
    ...
    def _choose_file(self)->None:
        path,_=QFileDialog.getOpenFileName(self,"Choose Coordinate File","","Coordinate files (*.kmz *.kml *.csv *.xlsx *.xls);;All files (*.*)")
        if path:self._load_path(path)

    def _load_path(self,path:str)->None:
        # Batch outputs contain Target X/Y/Z and Project Info. Load those target values directly.
    ...
    def _load_batch_converted_xlsx(self,path:str)->bool:
        try:
            ...
            ws=wb["Points"] if "Points" in wb.sheetnames else wb.active
            rows=list(ws.iter_rows(values_only=True)); info=wb["Project Info"] if "Project Info" in wb.sheetnames else None
            target_crs=None
            if info:
            ...
            required={"Point Name","Target X","Target Y"}
            if not required.issubset(idx): return False
            pts=[]
            for n,row in enumerate(rows[1:],1):
                try:
                    x=float(row[idx["Target X"]]); y=float(row[idx["Target Y"]]); z=None
                    if "Target Z" in idx and row[idx["Target Z"]] is not None: z=float(row[idx["Target Z"]])
                    name=str(row[idx["Point Name"]]) if row[idx["Point Name"]] is not None else f"PT-{n}"
                    status=str(row[idx["Status"]]) if "Status" in idx else "SUCCESS"
                    pts.append(PointResult(name,x,y,z,x,y,z,status=status))
                except (TypeError,ValueError,IndexError):
                    continue
            if not pts: return False
            self.batch_converted=True; self.source_points=pts; self.result_points=pts; self.current_file=path
            label=f"{Path(path).name} — {len(pts)} points — ALREADY CONVERTED"
            if target_crs: label += f" — {target_crs}"
            self.file_label.setText(label); self._set_crs_both(target_crs); self.progress.setValue(len(pts)); self.progress.setMaximum(len(pts)); self._populate()
            return True
        except Exception as exc:
            QMessageBox.critical(self,"Batch Result Error",f"Could not load batch-converted file:\n{exc}"); return False

    def _set_crs_both(self,epsg:str|None)->None:
        if not epsg: return
        try:
            info=self.engine.get_crs_info(epsg)
            name=getattr(info,"name",epsg)
        except Exception: name=epsg
        self.source_picker.set_selected(epsg,name); self.target_picker.set_selected(epsg,name)

    def _convert(self)->None:
        if self.batch_converted:
            QMessageBox.information(self,"Already Converted","This file was produced by Batch Converter and is already in the target CRS. Export it directly to DXF or Civil 3D CSV."); return
    ...
    def _populate(self)->None:
        pts=self.result_points; ok=sum(p.status=="SUCCESS" for p in pts); bad=sum(p.status=="FAILED" for p in pts); self.total.setText(f"Points: {len(pts)}"); self.success.setText(f"Success: {ok}"); self.failed.setText(f"Failed: {bad}"); self.table.setRowCount(len(pts))
        for i,p in enumerate(pts):
            for j,value in enumerate([p.name,p.tgt_x,p.tgt_y,p.tgt_z,p.status,p.message]): self.table.setItem(i,j,QTableWidgetItem("" if value is None else str(value)))
